...
- **Grid** — axial hex math (`Hex` — neighbour directions, cube conversion, distance, rounding), per-cell data (`HexCell`), map outlines (`IMapShape` with `HexagonShape` / `RectangleShape` / `ParallelogramShape`), and grid assembly + mesh/wall generation (`HexGridManager`, `HexMapGenerator` — plain C# classes, owned by `HexMap`)
...
- **Noise** — `Noise2D`, a `[SerializeReference]` polymorphic tree, all outputting [0, 1], composed inline on a `NoisePreset` — a reusable ScriptableObject (`Hills`, `Mountains`, `Steppe`…) that bundles the tree with its band count and feature size, which a `HexMap` points at and several maps can share. Primitives: `PerlinNoise`, `SimplexNoise`, `WorleyNoise`. Combiners that wrap other `Noise2D`s: `FractalNoise` (`Fbm` / `Ridged` / `Billow` octave stacking), `DomainWarpNoise` (offsets a source's sample position by a warp noise), and `CurveNoise` (remaps a source through an `AnimationCurve` — invert, contrast, terracing).
